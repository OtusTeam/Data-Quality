##load_raw_data_to_db_stage.py ##rename this DAG before using it
...
def load_sheet_to_db_stage(sheet_name, target_table, **context):
    ...
    try:
        ...
        if len(df) == 0:
            print(f"⚠️ Лист {sheet_name} пустой, пропускаем")
            return
...
with DAG(
    ...
    trigger_dq = TriggerDagRunOperator(
        task_id='trigger_dq_daily',
        trigger_dag_id='dq_daily',        # имя DAG, который нужно запустить
        wait_for_completion=False,         # False – не ждать завершения dq_daily
        dag=dag,
)
    
    # Порядок выполнения
    validate_task >> load_tasks >> verify_task >> trigger_dq
